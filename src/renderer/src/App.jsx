...
function App() {
  ...
  function handleAdministrationClick() {
    setMenuMessage('Administration screen integration comes next.')
    setMenuError('')
  }
  ...
  function handleCodingClick() {
    ...
    setMenuMessage(`Coding placeholder for ${selectedRow.participantLabel ?? 'selected record'}.`)
    setMenuError('')
  }
  ...
  function handleRecordsClick() {
    ...
    setMenuMessage(`Records placeholder for ${selectedRow.participantLabel ?? 'selected record'}.`)
    setMenuError('')
  }
  ...
  async function handleDeleteClick(row) {
    const confirmed = window.confirm(
      `Are you sure you want to remove ${row.participantLabel ?? 'the selected participant'}?`
    )

    if (!confirmed) return

    try {
      const result = await window.api?.deleteSession?.(row.sessionId)
